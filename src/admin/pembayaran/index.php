...
$query = mysqli_query($conn, "
    SELECT p.*, c.total, u.nama
    FROM pembayaran p
    JOIN checkout c ON p.id_checkout = c.id_checkout
...
<head>
    ...
    <title>Manajemen Pembayaran - Azta Es Cream</title>
    ...
    <div class="row">
        ...
        <div class="col-lg-10 col-md-9 main-content">
            <div class="content-header">
                <h1><i class="fas fa-credit-card me-2"></i>Manajemen Pembayaran</h1>
                <p>Kelola data pembayaran dan verifikasi transaksi untuk Azta Es Cream</p>
            </div>
                    ...
                    <table class="table-custom">
                        ...
                        <tbody>
                            <?php $no = 1; while ($row = mysqli_fetch_assoc($query)) { 
                                $method_class = 'badge-other';
                                $method_icon = 'fa-credit-card';
                                
                                if (strpos(strtolower($row['metode']), 'transfer') !== false) {
                                    $method_class = 'badge-transfer';
                                    $method_icon = 'fa-exchange-alt';
                                } else if (strpos(strtolower($row['metode']), 'cash') !== false || strpos(strtolower($row['metode']), 'tunai') !== false) {
                                    $method_class = 'badge-cash';
                                    $method_icon = 'fa-money-bill-wave';
                                } else if (strpos(strtolower($row['metode']), 'e-wallet') !== false || strpos(strtolower($row['metode']), 'dompet') !== false) {
                                    $method_class = 'badge-ewallet';
                                    $method_icon = 'fa-wallet';
                                }
                            ?>
                            <tr>
                                <td data-label="No"><?= $no++; ?></td>
                                <td data-label="Pelanggan"><?= htmlspecialchars($row['nama']); ?></td>
                                <td data-label="Total Bayar" class="payment-amount">
                                    Rp <?= number_format($row['total'], 0, ',', '.'); ?>
                                </td>
                                <td data-label="Metode">
                                    <span class="badge-custom <?= $method_class; ?>">
                                        <i class="fas <?= $method_icon; ?>"></i><?= $row['metode']; ?>
                                    </span>
                                </td>
                                <td data-label="Tanggal"><?= date('d-m-Y', strtotime($row['tanggal'])); ?></td>
                                <td data-label="Aksi">
                                    <div class="btn-action">
                                        <a href="edit.php?id=<?= $row['id_pembayaran']; ?>" class="btn-edit">
                                            <i class="fas fa-edit"></i> Edit
                                        </a>
                                        <a href="hapus.php?id=<?= $row['id_pembayaran']; ?>" class="btn-delete" onclick="return confirm('Yakin hapus data pembayaran ini?')">
                            ...
                            <?php if (mysqli_num_rows($query) == 0): ?>
                                <tr>
                                    <td colspan="6" class="text-center text-muted py-4">
                                        <i class="fas fa-receipt me-2"></i>Belum ada data pembayaran
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
